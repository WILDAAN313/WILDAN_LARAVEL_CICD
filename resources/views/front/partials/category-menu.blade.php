...
{{-- يمنع ظهور القائمة قبل تحميل Alpine --}}
<style>[x-cloak]{display:none!important}</style>

<ul class="{{ $level === 0 ? 'flex flex-col sm:flex-row items-center justify-center text-sm font-bold uppercase px-6 py-2' : 'mt-2 space-y-1 pl-3' }}">
            ...
            @if ($category->childrenRecursive->isNotEmpty())
                ...
                <div x-data="{ open: false }" class="sm:hidden">
                    <button @click="open = !open"
                            class="ml-2 text-xs underline align-middle">
